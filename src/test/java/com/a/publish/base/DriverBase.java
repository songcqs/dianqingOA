...
public class DriverBase {
	...
	public DriverBase(String browser) {
		...
		driver = selectDriver.selectBrowserDriver(browser);
	}
	...
	public DriverBase(DriverBase driverBase) {
	}
...
//		driver.get(url);
	...
	/*
	public Iterator<String> getWindowsHandless() {
		// Set里面不允许有重复的元素,检索元素效率低下，删除和插入效率高，插入和删除不会引起元素位置改变；
		Set<String> winHandels = driver.getWindowHandles();
		Iterator<String> handles = winHandels.iterator();
		return handles;
	}*/
...
//			driver.switchTo().window(current_handle);
	 ...
	 * 根据页面title切换新窗口
	 */
	public static boolean switchToWindowTitle(String windowTitle, WebDriver dr) {
		boolean flag = false;
		try {
...
//						System.out.println("Switch to window: " + windowTitle + " successfully!");
						System.out.println("切换到窗口: " + windowTitle + " 成功!");
						break;
					} else
						continue;
				}
...
//			System.out.printf("Window: " + windowTitle + " cound not found!", e.fillInStackTrace());
			System.out.printf("窗口: " + windowTitle + " 找不到!", e.fillInStackTrace());
			flag = false;
		}
		return flag;
	}
...
//		} catch (NoSuchElementException e) {
	...
	public void clearTextEle(WebElement element) {
		element.clear();
	}
	...
	public void sendKeys(WebElement element, String text) {
		if (element != null) {
			...
			System.out.println(element + "元素没有定位到,输入失败" + text);
		}
	}
	...
	// 方法2
	public void setElementValue2(WebElement element, String value) {
		element.sendKeys(Keys.HOME, Keys.chord(Keys.SHIFT, Keys.END), value);// method2
	}

	// 方法3
	public void setElementValue3(WebElement element, String value) {
		element.sendKeys(Keys.CONTROL + "a");// method3
		element.sendKeys(value);// method3
	}

	// 方法4
	public void setElementValue4(WebElement element, String value) {
		element.click();
		element.clear();
...
//		String executeFile = "E:\\Javaworkspace\\mail163Send\\source\\uplod.exe"; // 定义了upload.exe文件的路径
	...
	/**
	 * actionMoveElement 悬浮/悬停 到更多按钮实现
	 */
